...
async function main() {
  ...
  if (docs && docs.length > 0) {
    const personalDocs = docs.filter(d => d.silo?.toLowerCase() === 'personal');
    console.log(`Found ${personalDocs.length} personal docs.`);
    const silos = Array.from(new Set(docs.map(d => d.silo)));
    console.log('Available silos:', silos);
